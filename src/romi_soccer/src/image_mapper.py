import rospy
import roslib
from romi_soccer.msg import Homography, Coordinates4
from rospy.numpy_msg import numpy_msg
class ImageMapper():

    def __init__(self):
        self.corner = Coordinates4()
        self.mat = Homography()
        self.pub = None
        self.rate = rospy.Rate(10)
        init_sub()
        init_pub()

    def init_pub():
        pub = rospy.Publisher('mapper/homography',Homography,queue_size=10)
        while not rospy.is_shutdown():
            self.recalibrate()
            rospy.spinOnce()
            rospy.sleep()

    def init_sub():
        rospy.loginfo('Initializing subscriber...')
        rospy.Subscriber('mapper/raw_data',Coordinates4,callback)
        rospy.loginfo('Done.')
        # Recalibrates homography matrix with new corner data

    def callback(data):
        rospy.loginfo('Received new coordinates from JSON.')
        rospy.loginfo('Recalibrating...')
        self.recalibrate()

    def recalibrate():
        u1 = corner.TopL.x + 32
        v2 = corner.TopL.y + 32
        x1 = 0
        y1 = 0

        u2 = corner.BotL.x - 32
        v2 = corner.BotL.y + 32
        x2 = 5
        y2 = 0

        u3 = corner.TopR.x + 32
        v3 = corner.TopR.y - 32
        x3 = 5
        y3 = 10

        u4 = corner.BotR.x - 32
        v4 = corner.BotR.y - 32
        x4 = 0
        y4 = 10

        A = numpy.array([x1, y1, 1, 0 , 0, 0, -u1*x1, -u1*y1],
        [0, 0, 0, x1, y1, 1, -v1*x1, -v1*y1],
        [x2, y2, 1, 0, 0, 0, -u2*x2, -u2*y2],
        [0, 0, 0, x2, y2, 1, -v2*x2, -v2*y2],
        [x3, y3, 1, 0, 0, 0, -u3*x3, -u3*y3],
        [0, 0, 0, x3, y3, 1, -v3*x3, -v3*y3],
        [x4, y4, 1, 0, 0, 0, -u4*x4, -u4*y4],
        [0, 0, 0, x4, y4, 1, -v4*x4, -v4*y4])

        b = numpy.array([u1],
        [v1],
        [u2],
        [v2],
        [u3],
        [v3],
        [u4],
        [v4])
        x = numpy.linalg.solve(A,b)

        mat[0] = x[0,0]
        mat[1] = x[1,0]
        mat[2] = x[2,0]
        mat[3] = x[3,0]
        mat[4] = x[4,0]
        mat[5] = x[5,0]
        mat[6] = x[6,0]
        mat[7] = x[7,0]

        pub.publish(mat)

if __name__=='__main__':
    try:
        map = ImageMapper()
    except rospy.ROSInterruptException:
        pass
